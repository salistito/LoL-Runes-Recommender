...
def get_runes(champion_name, runes_sets_json_file):
    """
    Retorna una lista con los sets de runas del campeón especificado
    """
...
def get_rune_set(set, runes_sets_json_file):
    """
    Añade a una variable de tipo string toda la información del set de runas
    y lo retorna para posteriormente mostrarlo en la GUI
    """
...
# especificar y posicionar texto descriptor de la imagen ingresada por el usuario
cropped_image_text = Label(root, text='Imagen Ingresada')
cropped_image_text.place(x=10, y=10)

# especificar y posicionar texto descriptor de la imagen encontrada por la aplicación
...
# especificar y posicionar imagen ingresada por el usuario
buscado = Label(root, image=cropped_image)
buscado.place(x=10, y=80)  # y=40

# especificar y posicionar imagen encontrada por la aplicación
resultado = Label(root, image=result_image)
resultado.place(x=210, y=40)

# Pagination de la interfaz grafica
pages = IntVar()
pages.set(0)
...
# Función para ejecutar el pagination de la app y desplegar el resto de la información en distintas paginas
